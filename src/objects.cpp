...
void Objects::initObjects()
{
        ...
        explodeShader->initShader(explodeVertex, explodeFragment, "asterbanger.bin");
        cloudShader = new Shader();
        cloudShader->initShader(cloudVertex, cloudFragment, "astercloud.bin");
        figure = new Model(modelinfo, shader, explodeShader, cloudShader, 2, QUANTITY);
        debug();
}
...
void Objects::drawObjects(mat4 model, mat4 view, mat4 projection, vec3 viewPos)
{
        ...
        figure->DrawInstanced(view, projection, modelinfo, modelData, lights, spotLights, viewPos, totals);
